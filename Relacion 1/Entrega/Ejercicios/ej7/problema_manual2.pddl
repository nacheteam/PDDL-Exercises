(define (problem Problema7)
  ...
  (:init
    ...
    ; Declaramos la orientación del agente 2 y que tiene la mano y la mochila
    (orientado agente2 norte)
    (manovacia agente2)
    (mochilavacia agente2)
    (en agente2 z10)

    ; Declaramos que el agente 1 es picker y el 2
    (espicker agente1)
    (esdealer agente2)
  ...
  (:goal (AND
            ...
            (>= (puntosTotales) 20)
            ; Requerimos que el dealer obtenga al menos 15 puntos.
            (>= (puntosObtenidos agente2) 15)
          )
  )
)
