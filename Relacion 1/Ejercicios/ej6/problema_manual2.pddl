(define (problem Problema6)
  ...
  (:init
    ...
    ; Declaramos el número de puntos de cada agente a 0
    (= (puntosObtenidos agente1) 0)
    (= (puntosObtenidos agente2) 0)
    ...
    ; Declaramos la orientación del agente 1 y que tiene la mano y la mochila vacías
    (orientado agente1 norte)
    (manovacia agente1)
    (mochilavacia agente1)
    (en agente1 z1)

    ; Declaramos la orientación del agente 2 y que tiene la mano y la mochila vacías
    (orientado agente2 norte)
    (manovacia agente2)
    (mochilavacia agente2)
    (en agente2 z10)
  ...
  (:goal (AND
            ...
            ; Requerimos que la longitud total sea menor que
            (< (costeTotal) 1500)
            ; Requerimos que el número de puntos totales sea mayor que 30
            (>= (puntosTotales) 30)
            ; Requerimos que el agente 1 tenga al menos 25 puntos y el dos al menos
            ; 30 de forma que superen los 30 puntos totales que se necesitan para
            ; superar el requisito anterior.
            (>= (puntosObtenidos agente1) 25)
            (>= (puntosObtenidos agente2) 30)
          )
  )
)
